import React, { useState, useMemo, forwardRef, useImperativeHandle, createContext, useContext } from 'react';
...
export interface TableAction { 
  ...
  icon: string; 
  title: string; 
  handler?: (rowId: any) => void; 
  type?: 'button' | 'link'; 
  to?: string; 
  getParams?: (row: any) => Record<string, string>; 
}
...
// --- CONTEXT ---
interface TableManagerContextValue {
  data: any[];
  columns: TableColumn[];
  actions: TableAction[];
  filters: FilterRule[];
...
const TableManagerContext = createContext<TableManagerContextValue | null>(null);

const useTableManager = () => {
  const context = useContext(TableManagerContext);
  if (!context) {
    throw new Error('useTableManager must be used within TableManagerProvider');
  }
  return context;
};
...
const FilterModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  tempFilters: Partial<FilterRule>[];
  setTempFilters: React.Dispatch<React.SetStateAction<Partial<FilterRule>[]>>;
}> = ({ isOpen, onClose, tempFilters, setTempFilters }) => {
  const { onStateChange } = useTableManager();
  
  const applyFilters = () => {
    onStateChange({ 
      filters: tempFilters.filter(f => f.column && f.value) as FilterRule[], 
      page: 1 
    });
    onClose();
  };
    ...
    <Modal 
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {SVGIcons.filter} Filter Table
        </div>
      } 
      footer={
        <>
          <button className="secondary-btn" onClick={() => setTempFilters([{}])}>
            Reset
          </button>
...
const SortModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  tempSorters: Partial<SortRule>[];
  setTempSorters: React.Dispatch<React.SetStateAction<Partial<SortRule>[]>>;
}> = ({ isOpen, onClose, tempSorters, setTempSorters }) => {
  const { onStateChange } = useTableManager();
  ...
  return (
    <Modal 
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {SVGIcons.sort} Sort Table
        </div>
      } 
      footer={
        <>
          <button className="secondary-btn" onClick={() => setTempSorters([{}])}>
            Reset
          </button>
...
const Pagination: React.FC = () => {
  const { currentPage, totalRecords, rowsPerPage, onStateChange } = useTableManager();
  const totalPages = useMemo(() => Math.ceil(totalRecords / rowsPerPage), [totalRecords, rowsPerPage]);
  
  const handlePageChange = (page: number) => {
    onStateChange({ page: Math.max(1, Math.min(page, totalPages || 1)) });
  };
...
const Table: React.FC = () => {
  const { data, columns, actions } = useTableManager();
...
const TableManager = forwardRef<any, TableManagerProps>(({
  ...
  const resetFilters = () => onStateChange({ filters: [] });
  const resetSorters = () => onStateChange({ sorters: [] });
  ...
  const contextValue: TableManagerContextValue = {
    data,
    columns,
    actions,
    filters,
  ...
  return (
    <TableManagerContext.Provider value={contextValue}>
      <div className={`table-container ${className}`}>
        <Table />
        <Pagination />
      </div>
